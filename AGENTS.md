...
- 2026-09-21: Running `generate_unit_tests_from_ragger all` purely as a drift check rewrote all 52 generated unit files, because `clang-format-14` (applied afterwards by `make -C tests clang-format-generated`) is not installed on this machine. Before regenerating, check whether `clang-format-14` is available; if it is not, determine drift by reasoning about which test lists `_load_sign_tx_tests()` actually consumes rather than regenerating, or be prepared to restore the generated files to their committed content.
