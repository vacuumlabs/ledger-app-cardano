...
bool buffer_read_bip44_path(buffer_t *buf, bip44_path_t* path)
{
    ...
    // Read length byte
    uint8_t length;
    if (!buffer_read_u8(buf, &length)) {
        return false;
    }

    // Validate length against BIP44 constraint (max 5 elements for Cardano)
    if (length > BIP44_MAX_PATH_ELEMENTS) {
        return false;
    }

    // Use standard buffer_read_bip32_path to read the path elements
    if (!buffer_read_bip32_path(buf, path->path, (size_t)length)) {
        return false;
    }
